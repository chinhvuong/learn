...
export function createErrorFactory<const T extends Record<string, ErrorDefinition<any>>>(
  errors: T,
) {
  const factories = {} as {
    [K in keyof T]: (...args: Parameters<T[K]['message']>) => AppError<Parameters<T[K]['message']>>;
  };
